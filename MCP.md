...
10. **process_memory_scrape** (CRITICAL / HIGH; CRITICAL is never demoted) -- a process obtained another process's task port or opened its memory (macOS Endpoint Security GET_TASK / GET_TASK_READ, Linux `ptrace_may_access` kprobe, `/proc/<pid>/mem` open); CRITICAL when the target is an AI agent or a credential holder (CI runner worker, ssh-agent, keychain, browser, password manager) or the requester's lineage is suspicious, HIGH for any other control-port / memory-open access; kernel-vouched platform binaries (ps, sysmond, system debuggers) and uncorroborated read-only ports are not reported
