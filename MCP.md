...
10. **process_memory_scrape** (CRITICAL / HIGH / LOW; CRITICAL is never demoted) -- a process obtained another process's task port or opened its memory (macOS Endpoint Security GET_TASK, Linux `ptrace_may_access` kprobe, `/proc/<pid>/mem` open); CRITICAL when the target is an AI agent or a credential holder (CI runner worker, ssh-agent, keychain, browser, password manager) or the requester's lineage is suspicious, LOW for a kernel-vouched platform binary such as a system debugger
